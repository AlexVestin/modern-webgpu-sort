R"(
  ...
  fn merge_path_2(a_keys: i32, a_count: i32, b_keys: i32, b_count: i32, diag: i32) -> i32 {
    ...
    loop {
      ...
      let b_key = shared[u32(b_keys) + u32(diag) - 1u - mid];
  ...
  fn segmented_merge_path(range: vec4<i32>, active: vec2<i32>, diag: i32) -> vec3<i32> {
    ...
    if (range.x + diag <= active.x)  {
      return vec3<i32>(diag, active.x, active.y);
    }

    if (range.x + diag >= active.y) {
      return vec3<i32>(range.y - range.x, active.x, active.y);
    } 
  ...
  fn shared_to_mem(tid: u32, count: u32, first: u32) {
    if (count <= 128u * 15u) {
      for(var i = 0u; i < 15u; i = i + 1u) {
        let j = 128u * i + tid;
        keys_dst.data[first + j] = shared[j];
      }
    } else {
      for(var i = 0u; i < 15u; i = i + 1u) {
        let j = 128u * i + tid;
  ...
  fn main(
    ...
    let pass = (pass_counter.data / params.num_partition_ctas) - 1u;
    let nv = 15u * 128u;    
    ...
    let list_parity = 1u & (u32(tile) >> pass);
    if (list_parity != 0u) {
      active = vec2<i32>(0, active.x);
    } else {
      active = vec2<i32>(active.y, i32(nv));
    } 
    ...
    var sort_warp: bool;
    if (list_parity != 0u)  {
      sort_warp = i32(warp_offset) < active.y;
    } else {
      sort_warp = i32(warp_offset + 15u * warp_size) >= active.x;
    }  
    ...
    if (sort_warp) {
      diag = 15u * tid;
      let ret = segmented_merge_path(local_range, active, i32(diag));
      mp = ret.x;
      //active = vec2<i32>(ret.y, ret.z);
      partitioned = partition(local_range, i32(mp), i32(diag));
      segmented_serial_merge(partitioned, active);
    }
